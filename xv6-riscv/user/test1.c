...
main(int _ac, char *_av[])
{
  ...
  rv = getiostats(fd2, &st);
  fprintf(1, "fd2a stats(%d) (r: %d, w: %d) -> %d\n", fd2, st.read_bytes, st.write_bytes, rv);
  ...
  rv = getiostats(fd1, &st);
  fprintf(1, "fd1r stats(%d) (r: %d, w: %d) -> %d\n", fd1, st.read_bytes, st.write_bytes, rv);

  rv = getiostats(fd2, &st);
  fprintf(1, "fd2r stats(%d) (r: %d, w: %d) -> %d\n", fd2, st.read_bytes, st.write_bytes, rv);

  read(fd1, buf, 128);

  rv = getiostats(fd1, &st);
  fprintf(1, "fd1ra stats(%d) (r: %d, w: %d) -> %d\n", fd1, st.read_bytes, st.write_bytes, rv);

  close(fd2);
  close(fd1);

  fprintf(1, "test1 done\n");
  exit(0);
  return 0;
}
